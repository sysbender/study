...
#include "Dude.h"
 ...
 Dude::Dude(int x, int y) : mLocation(x, y), mRect(20, 20)
{
}

 void Dude::move(Point dest)
{
	mLocation = dest;
}
 ...
 bool Dude::IsColliding(Poo poo) const
{
	...
	Rect r = poo.rect;
	return mLocation.x <= p.x + r.width
		&& mLocation.x + mRect.width >= p.x
		&& mLocation.y <= p.y + r.height
		&& mLocation.y + mRect.height >= p.y;
}
