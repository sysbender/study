...
public abstract class ShaderProgram {
	...
	private static FloatBuffer matrixBuffer = BufferUtils.createFloatBuffer(16)	; // matrix 4X4
	...
	// get the location of uniform vairable 
	public int getUniformLocation(String uniformName) {
		return GL20.glGetUniformLocation(programID, uniformName);
		
	}
	
	// load a float value to a location
	protected void loadFloat(int location, float value) {
		
		GL20.glUniform1f(location, value);
	}
	
	// load a vector to a location
	
	protected void loadVector(int location , Vector3f vector) {
		GL20.glUniform3f(programID, vector.x, vector.y, vector.z);
	}
	
	// load boolean as float
